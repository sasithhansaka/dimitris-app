...
export function ArticleCardHome({ article, featured = false }: { article: Article; featured?: boolean }) {
  ...
  return (
    ...
    >
      <div className={featured ? "min-h-65 overflow-hidden" : "aspect-video overflow-hidden"}>
        {article.banner ? (
          <img src={`/storage/${article.banner}`} alt={article.title} className="h-full w-full object-cover" />
        ) : (
