...
export function CouponCard({ coupon }: { coupon: Coupon }) {
  return (
    <article className="u-lift flex h-full flex-col overflow-hidden rounded-md border border-rule bg-surface">
      <Link href={`/coupons/${coupon.slug}`} className="group block focus-visible:-outline-offset-2">
        <div className="relative aspect-16/10 overflow-hidden bg-paper-deep">
          ...
          <div className="absolute top-3 left-3 max-w-[75%] rounded-sm bg-white px-3 py-2 shadow-[0_1px_3px_rgba(20,21,28,0.12)]">
            ...
            <p className="u-label mt-1 text-brand-ink">{coupon.rewardKind}</p>
          </div>
        </div>
      </Link>
      ...
      <div className="flex flex-1 flex-col p-4">
        <div className="flex items-center gap-2">
          ...
          <span className="u-label truncate text-ink-2">{coupon.brand.name}</span>
        ...
        {coupon.productName && (
          <p className="mt-1 line-clamp-1 text-[0.81rem] text-ink-3">{coupon.productName}</p>
        )}
        ...
        <div className="mt-3">
          <CouponActions couponId={coupon.id} title={coupon.title} reward={coupon.reward} stacked />
        </div>
      </div>
    </article>
  );
