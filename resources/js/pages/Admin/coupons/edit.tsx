...
export default function EditCoupon({
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        products: (coupon.products && coupon.products.length > 0
            ? coupon.products.map((product) => ({
                  product_id: String(product.id),
                  ocr_name: product.pivot.ocr_name,
                  ocr_keywords: product.pivot.ocr_keywords,
                  quantity: String(product.pivot.quantity),
                  required: Boolean(product.pivot.required),
              }))
            : [emptyProduct()]) as CouponProductForm[],
        retailer_ids: linkedRetailerIds,
        remove_image: false,
        _method: 'put',
    });

    const originalImage = coupon.image ? `/storage/${coupon.image}` : null;
    ...
    const previousCountry = useRef(coupon.country);
    ...
    useEffect(() => {
        if (data.country === previousCountry.current) {
            return;
        }
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(couponsRoutes.update(coupon.id).url, { forceFormData: true });
    };
                ...
                <div className="mx-auto w-full max-w-5xl">
                    <div className="mb-5 space-y-1">
                        ...
                        <p className="text-muted-foreground text-sm">
                            Update this coupon's details, products and
                            eligible retailers.
                        </p>
                    </div>
